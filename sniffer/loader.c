...
static const char *default_filename = "kern.o";
...
const char *queue_name = "xdp_packets_queue";
...
int main(int argc, char **argv)
{
	...
	if (!cfg.reuse_maps)
	{
		err = pin_maps_in_bpf_object(xdp_program__bpf_obj(program), cfg.ifname, map_name);
		if (err)
		{
		...
		err = pin_maps_in_bpf_object(xdp_program__bpf_obj(program), cfg.ifname, queue_name);
		if (err)
		{
			fprintf(stderr, "ERR: pinning queue\n");
			return err;
		}
	}
