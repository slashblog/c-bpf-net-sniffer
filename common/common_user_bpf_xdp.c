...
struct xdp_program *load_bpf_and_xdp_attach(struct config *cfg)
{
	...
	if (cfg->reuse_maps) {
		...
		int len = snprintf(map_filename, PATH_MAX, "%s/%s/%s", cfg->pin_dir, cfg->ifname, cfg->map_name);
		if (len < 0) {
		...
		struct bpf_map *map = bpf_object__find_map_by_name(bpf_obj, cfg->map_name);
...
int check_queue_fd_info(const struct bpf_map_info *info,
		      const struct bpf_map_info *exp)
{
	if (exp->value_size && exp->value_size != info->value_size) {
		fprintf(stderr, "ERR: %s() "
